...
import POM from "../Company/POM.jsx";
...
class UserProfile extends Component {
  ...
  handleFollowingAction = () => {
    ...
    if (is_a_follower) {
      ...
      axios
        .post(
          `${API}follow-designer/${this.props.info?.id}/${this.state.user?.id}`
        )
        .then((response) => {
          console.log(response);
          this.setState({
      ...
      <React.Fragment>
                    ...
                    {!parseInt(this.state.is_designer) ? (
                      ...
                      <>
                        <p className="dc mb-0">
                          ...
                          <span>
                            <Tooltip
                              title={this.state.copied ? "Copied" : "Copy Code"}
                            >
                              <Button
                                onClick={() =>
                        ...
                        {this.state.user?.country &&
                          this.state.user?.country?.length > 0 && (
                            <p className="loc">
                              <EnvironmentFilled />
                        ...
                        <div className="professions">
                          {this.state.user?.professions?.map((p) => {
                            return (
                              <div>
                                <a
                                  href={`/designers?filterBy=designer&profession=${p}`}
                                >
                                  <p>{p}</p>
                                </a>
                              </div>
                      ...
                      <>
                        <Col md={{ span: 4 }}>
                          <button
                            ...
                            type="button"
                            style={{
                              backgroundColor: this.state.is_a_follower
                                ? "#e8e8e8"
                                : "",
                            }}
                            title={
                              this.state.is_a_follower ? "Unfollow" : "Follow"
                            }
                  ...
                  >
                    <TabList>
                      ...
                      {this.state.products?.length > 0 && (
                        <Tab tabIndex={4}>Products</Tab>
                      )}
                      {!this.state.visitor && (
                        <Tab>
                          Company Pages{" "}
                          <span className="tab-icon tab-icon-info">i</span>
                        </Tab>
                      )}
                      ...
                      {!this.state.visitor && <Tab>PO Managament</Tab>}
                    </TabList>
                    {this.state.user?.categories?.length > 0 && (
                      <TabPanel>
                        ...
                        <AntRow
                          span={24}
                          gutter={{ md: 24, lg: 24, xs: 12, sm: 12 }}
                        >
                        ...
                        >
                          {!this.state.visitor && (
                            <AntCol
                              ...
                              sm={12}
                              md={8}
                              className="my-4 add-col"
                            >
                              <div className="add-project-icon">
                                <a
                                  href={`/addproject/designer/${this.state.user?.id}`}
                                >
                                  <AiOutlinePlus />
                                  Add Project
                                </a>
                          ...
                          {this.state.projects?.length +
                            ...
                            <>
                              ...
                              {this.state.taggedProjects?.map((p, index) => {
                                return (
                                  <>
                                    {!this.state.projectsIds?.includes(
                                      p.id
                                    ) && (
                                      <AntCol
                                        xs={12}
                                          ...
                                          >
                                            ...
                                            {product?.files?.length > 0 ? (
                                              <>
                                                ...
                                                <div className="actns-btn file-btn threeD">
                                                  3D
                                                </div>
                                              </>
                                            ) : (
                      ...
                      <TabPanel>
                        <POM
                          company_cashflow_base_currency={
                            this.state.user?.cashflow_base_currency
                          }
                          entity_id={this.state.user?.id}
                          entity_name="user"
                        />
                      </TabPanel>
                    )}
        ...
        <AntModal
          ...
          closeIcon={
            <>
              <div
                onClick={() => this.setState({ save_to_board_modal: false })}
              >
                X
              </div>
            </>
