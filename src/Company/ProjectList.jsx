...
class ProjectList extends Component {
  ...
  filterList = () => {
    // const name_filter=this.state.name
    const {
      year_filter,
      status_filter,
      country_filter,
    ...
    axios
      .get(
        `${API}projectlists-filter/${this.state.entity_id}?filter[status]=${status_filter}&filter[country]=${country_filter}&filter[name]=${name_filter}&filter[client_name]=${client_filter}&filter[start_date]=${year_filter}`
      )
      .then((response) => {
        this.setState({
          project_rows: response.data.projects,
  ...
  editProjectValuesChange =(changedValues, allValues) => {
    ...
    project_to_edit.business_type=
    changedValues.business_type
    ...
    return (
      <>
                ...
                {this.state.loading ? (
                  ...
                  <div className="table-wrapper">
                    <table id="client-lists">
                      <tr>
                        <th className="width-300">Name, NO</th>
                        <th className="width-200">Customer</th>
                        <th className="width-200">Location</th>
                        <th className="width-200">Start Date</th>
                        <th className="width-200">Total Value</th>
                        <th className="width-150">Statue</th>
                      </tr>
                      {this.state.project_rows?.map((row, index) => {
                        return (
                          <tr className="white light-hover">
                            ...
                            >
                              <p className="main">{row?.start_date}</p>
                            </td>
                            <td
                             onClick={() => {
                              this.setState(
                            ...
                            >
                              <p className="main">
                                {Number(parseFloat(row?.total).toFixed(2)).toLocaleString()}
                              </p>
          ...
          >
            ...
            <Form onFinish={this.onFinishEdit}
            ...
            size="large" layout="vertical">
              ...
              >
                <Select
                disabled={this.state.project_to_edit?.business_type=='Purchases Only'|| this.state.edit_client_disabled}
